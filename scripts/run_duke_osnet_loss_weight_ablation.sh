#!/usr/bin/env bash
# Run DukeMTMC-reID loss-weight ablations for MambaVision + OSNet descriptor fusion.
# Usage:
#   bash scripts/run_duke_osnet_loss_weight_ablation.sh 0,1 2
#   bash scripts/run_duke_osnet_loss_weight_ablation.sh 0,1 2 ./logs/Duke/my_output
#   bash scripts/run_duke_osnet_loss_weight_ablation.sh summary ./logs/Duke/my_output
...
CONFIG="${CONFIG:-configs/DukeMTMC/mambavision_tiny_osnet_concat_b64k4.yml}"
OUTPUT_BASE="${OUTPUT_BASE:-${OUTPUT_BASE_ARG:-./logs/Duke/osnet_loss_weight_ablation}}"
...
run_experiment() {
  local idx="$1"
  local spec="$2"
  local name osnet_weight fused_weight

  IFS='|' read -r name osnet_weight fused_weight <<< "${spec}"
  ...
  echo "[DukeOSNetLoss] GPU=${gpu} EXP=${name} output=${output_dir} osnet_w=${osnet_weight} fused_w=${fused_weight}"

  CUDA_VISIBLE_DEVICES="${gpu}" python train.py --config_file "${CONFIG}" \
    MODEL.DEVICE_ID "'${gpu}'" \
    MODEL.OSNET_FUSION.ENABLED True \
    MODEL.OSNET_FUSION.FUSION_TYPE "'descriptor'" \
...
summarize_results() {
  ...
  specs="$(printf "%s\n" "${EXPERIMENTS[@]}")"
  echo "[DukeOSNetLoss] Summarizing results -> ${summary_file} and ${summary_csv}"

  DUKE_OSNET_LOSS_SPECS="${specs}" python - "${OUTPUT_BASE}" "${summary_file}" "${summary_csv}" <<'PY'
import csv
import os
import re
import sys
...
specs = [line for line in os.environ.get("DUKE_OSNET_LOSS_SPECS", "").splitlines() if line.strip()]
...
rows = []
for spec in specs:
    name, osnet_weight, fused_weight = spec.split("|")
...
print()
print(
    f"{'name':<14} {'status':<12} {'osw':<6} {'fuw':<6} "
    f"{'last_ep':<8} {'last_mAP':<8} {'last_R1':<8} "
    f"{'best_ep':<8} {'best_mAP':<8} {'best_R1':<8}"
)
for row in rows:
    print(
        f"{row['name']:<14} {row['status']:<12} {row['osnet_weight']:<6} {row['fused_weight']:<6} "
...
if [ "${SUMMARY_ONLY}" -eq 1 ]; then
  echo "[DukeOSNetLoss] OUTPUT_BASE=${OUTPUT_BASE}"
  summarize_results
  exit 0
fi

echo "[DukeOSNetLoss] OUTPUT_BASE=${OUTPUT_BASE}"
echo "[DukeOSNetLoss] Summary files: ${SUMMARY_TSV}, ${SUMMARY_CSV}"
...
summarize_results
echo "[DukeOSNetLoss] All experiments finished."

if [ "${failures}" -ne 0 ]; then
  echo "[DukeOSNetLoss] One or more experiments failed. Check the logs above and ${OUTPUT_BASE}."
  exit 1
fi
