#!/usr/bin/env bash
# Ablate Stage4 pre-FDMF part-wise stripe local fusion on DukeMTMC-reID.
# Usage:
#   bash scripts/run_duke_osnet_fdmf_s4stripe_local.sh 0,1 2
#   bash scripts/run_duke_osnet_fdmf_s4stripe_local.sh 0,1 2 ./logs/Duke/my_output
#   bash scripts/run_duke_osnet_fdmf_s4stripe_local.sh summary ./logs/Duke/my_output
set -euo pipefail
...
OUTPUT_BASE="${OUTPUT_BASE:-${OUTPUT_BASE_ARG:-./logs/Duke/osnet_fdmf_s4stripe_local}}"
...
declare -a EXPERIMENTS=(
  ...
  "s4psf_k2_w01_d1_shared|True|2|0.1|1|True|0"
  "s4psf_k2_w02_d1_shared|True|2|0.2|1|True|0"
  "s4psf_k2_w03_d1_shared|True|2|0.3|1|True|0"
  "s4psf_k4_w01_d1_shared|True|4|0.1|1|True|0"
  "s4psf_k4_w02_d1_shared|True|4|0.2|1|True|0"
  "s4psf_k4_w03_d1_shared|True|4|0.3|1|True|0"
  "s4psf_k8_w01_d1_shared|True|8|0.1|1|True|0"
  "s4psf_k8_w02_d1_shared|True|8|0.2|1|True|0"
  "s4psf_k8_w03_d1_shared|True|8|0.3|1|True|0"
  "s4psf_k4_w02_d0_shared|True|4|0.2|0|True|0"
  "s4psf_k4_w02_d2_shared|True|4|0.2|2|True|0"
  "s4psf_k8_w02_d0_shared|True|8|0.2|0|True|0"
  "s4psf_k8_w02_d2_shared|True|8|0.2|2|True|0"
  "s4psf_k2_w02_d1_indep|True|2|0.2|1|False|0"
  "s4psf_k4_w02_d1_indep|True|4|0.2|1|False|0"
  "s4psf_k8_w02_d1_indep|True|8|0.2|1|False|0"
  "s4psf_k2_w02_d1_p128|True|2|0.2|1|True|128"
  "s4psf_k4_w02_d1_p128|True|4|0.2|1|True|128"
  "s4psf_k8_w02_d1_p128|True|8|0.2|1|True|128"
)
...
run_experiment() {
  ...
  if [ "${SKIP_COMPLETED}" = "1" ] && [ -f "${train_log}" ] && grep -q "Validation (Regular) Results - Epoch:[[:space:]]*${COMPLETE_EPOCH}" "${train_log}"; then
    echo "[DukeS4PSF] SKIP completed EXP=${name} output=${output_dir} epoch=${COMPLETE_EPOCH}"
    return 0
  fi
  ...
  echo "[DukeS4PSF] GPU=${gpu} EXP=${name} output=${output_dir} enabled=${enabled} k=${num_stripes} loss_w=${loss_weight} depth=${mamba_depth} share=${share_params} part_dim=${part_dim} infer_w=${INFER_WEIGHT}"
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" python train.py --config_file "${CONFIG}" \
    ...
    MODEL.OSNET_FUSION.STAGE3_STRIPE_LOCAL_ENABLED False \
    MODEL.OSNET_FUSION.STAGE4_STRIPE_LOCAL_ENABLED "${enabled}" \
    MODEL.OSNET_FUSION.STAGE4_STRIPE_LOCAL_NUM_STRIPES "${num_stripes}" \
    MODEL.OSNET_FUSION.STAGE4_STRIPE_LOCAL_LOSS_WEIGHT "${loss_weight}" \
    MODEL.OSNET_FUSION.STAGE4_STRIPE_LOCAL_INFER_WEIGHT "${INFER_WEIGHT}" \
    MODEL.OSNET_FUSION.STAGE4_STRIPE_LOCAL_MAMBA_DEPTH "${mamba_depth}" \
    MODEL.OSNET_FUSION.STAGE4_STRIPE_LOCAL_SHARE_PARAMS "${share_params}" \
    MODEL.OSNET_FUSION.STAGE4_STRIPE_LOCAL_PART_DIM "${part_dim}" \
    TEST.FEAT_MODE "'mamba_fdmf_osnet_stage4local'" \
...
summarize_results() {
  local specs
  specs="$(printf "%s\n" "${EXPERIMENTS[@]}")"
  echo "[DukeS4PSF] Summarizing results -> ${SUMMARY_TSV} and ${SUMMARY_CSV}"

  DUKE_S4PSF_SPECS="${specs}" python - "${OUTPUT_BASE}" "${SUMMARY_TSV}" "${SUMMARY_CSV}" <<'PY'
import csv
import os
import re
import sys
...
specs = [line for line in os.environ.get("DUKE_S4PSF_SPECS", "").splitlines() if line.strip()]
...
if [ "${SUMMARY_ONLY}" -eq 1 ]; then
  echo "[DukeS4PSF] OUTPUT_BASE=${OUTPUT_BASE}"
  summarize_results
  exit 0
fi

echo "[DukeS4PSF] OUTPUT_BASE=${OUTPUT_BASE}"
echo "[DukeS4PSF] Summary files: ${SUMMARY_TSV}, ${SUMMARY_CSV}"
echo "[DukeS4PSF] SKIP_COMPLETED=${SKIP_COMPLETED} COMPLETE_EPOCH=${COMPLETE_EPOCH} INFER_WEIGHT=${INFER_WEIGHT}"
...
summarize_results
echo "[DukeS4PSF] All experiments finished."

if [ "${failures}" -ne 0 ]; then
  echo "[DukeS4PSF] One or more experiments failed. Check the logs above and ${OUTPUT_BASE}."
  exit 1
fi
