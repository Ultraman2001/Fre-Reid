#!/usr/bin/env bash
# Train DukeMTMC-reID MambaVision + OSNet raw concat fusion with OSNet loss weight 1.0.
# Usage:
#   bash scripts/run_duke_osnet_raw_concat_osw1.sh 0,1 2
#   bash scripts/run_duke_osnet_raw_concat_osw1.sh 0,1 2 ./logs/Duke/my_output
#   bash scripts/run_duke_osnet_raw_concat_osw1.sh summary ./logs/Duke/my_output
...
declare -a EXPERIMENTS=(
  "raw_concat_osw1_fuw1|none|1.0|1.0|1.0"
)
...
run_experiment() {
  ...
  echo "[DukeOSNetRaw] GPU=${gpu} EXP=${name} output=${output_dir} norm=${fusion_norm} osnet_w=${osnet_weight} fused_w=${fused_weight}"
...
summarize_results() {
  ...
  specs="$(printf "%s\n" "${EXPERIMENTS[@]}")"
  echo "[DukeOSNetRaw] Summarizing results -> ${summary_file} and ${summary_csv}"

  DUKE_OSNET_RAW_SPECS="${specs}" python - "${OUTPUT_BASE}" "${summary_file}" "${summary_csv}" <<'PY'
import csv
import os
import re
import sys
...
specs = [line for line in os.environ.get("DUKE_OSNET_RAW_SPECS", "").splitlines() if line.strip()]
...
print()
print(
    f"{'name':<24} {'status':<12} {'norm':<8} {'osw':<5} {'fuw':<5} "
    f"{'last_ep':<8} {'last_mAP':<8} {'last_R1':<8} "
    f"{'best_ep':<8} {'best_mAP':<8} {'best_R1':<8}"
)
for row in rows:
    print(
        f"{row['name']:<24} {row['status']:<12} {row['fusion_norm']:<8} "
...
if [ "${SUMMARY_ONLY}" -eq 1 ]; then
  echo "[DukeOSNetRaw] OUTPUT_BASE=${OUTPUT_BASE}"
  summarize_results
  exit 0
fi

echo "[DukeOSNetRaw] OUTPUT_BASE=${OUTPUT_BASE}"
echo "[DukeOSNetRaw] Summary files: ${SUMMARY_TSV}, ${SUMMARY_CSV}"
...
summarize_results
echo "[DukeOSNetRaw] All experiments finished."

if [ "${failures}" -ne 0 ]; then
  echo "[DukeOSNetRaw] One or more experiments failed. Check the logs above and ${OUTPUT_BASE}."
  exit 1
fi
