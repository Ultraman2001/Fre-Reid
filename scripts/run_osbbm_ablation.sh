#!/usr/bin/env bash
# Run OSBBM augmentation ablations, two experiments at a time by default.
set -euo pipefail
...
CONFIG="${CONFIG:-configs/OCC_Duke/mambavision_tiny_transreid_pam_padeaug_osbbm_b64k4.yml}"
OUTPUT_BASE="${OUTPUT_BASE:-./logs/OCC-Duke/osbbm_ablation}"
...
declare -a EXPERIMENTS=(
  "osbbm_p015_b8_m4_base|0.15|8|4|0.50|base"
  "osbbm_p025_b8_m4_base|0.25|8|4|0.50|base"
  "osbbm_p050_b8_m4_base|0.50|8|4|0.50|base"
  "osbbm_p025_b8_m2_base|0.25|8|2|0.50|base"
  "osbbm_p025_b8_m6_base|0.25|8|6|0.50|base"
  "osbbm_p025_b8_m4_all|0.25|8|4|0.50|all"
)

run_experiment() {
  local idx="$1"
  local spec="$2"
  local name prob blocks mix_blocks gray_prob apply_to

  IFS='|' read -r name prob blocks mix_blocks gray_prob apply_to <<< "${spec}"

  local gpu="${GPUS[$((idx % ${#GPUS[@]}))]}"
  local output_dir="${OUTPUT_BASE}/${name}"

  echo "[OSBBM] GPU=${gpu} EXP=${name} prob=${prob} blocks=${blocks} mix=${mix_blocks} gray=${gray_prob} apply=${apply_to}"
...
summarize_results() {
  ...
  specs="$(printf "%s\n" "${EXPERIMENTS[@]}")"
  echo "[OSBBM] Summarizing results -> ${summary_file} and ${summary_csv}"

  OSBBM_SPECS="${specs}" python - "${OUTPUT_BASE}" "${summary_file}" "${summary_csv}" <<'PY'
import csv
import os
import re
import sys
...
specs = [line for line in os.environ.get("OSBBM_SPECS", "").splitlines() if line.strip()]
...
rows = []
for spec in specs:
    name, prob, blocks, mix_blocks, gray_prob, apply_to = spec.split("|")
...
print()
print(
    f"{'name':<26} {'status':<12} {'prob':<6} {'mix':<5} {'apply':<6} "
    f"{'last_ep':<8} {'last_mAP':<8} {'last_R1':<8} "
    f"{'best_ep':<8} {'best_mAP':<8} {'best_R1':<8}"
)
for row in rows:
    print(
        f"{row['name']:<26} {row['status']:<12} {row['prob']:<6} {row['mix_blocks']:<5} {row['apply_to']:<6} "
...
summarize_results
echo "[OSBBM] All experiments finished."

if [ "${failures}" -ne 0 ]; then
  echo "[OSBBM] One or more experiments failed. Check the logs above and ${OUTPUT_BASE}."
  exit 1
fi
