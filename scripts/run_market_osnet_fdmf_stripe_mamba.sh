#!/usr/bin/env bash
# Ablate stripe-local + global map-Mamba fusion on Market1501 without OSBBM.
# Usage:
#   bash scripts/run_market_osnet_fdmf_stripe_mamba.sh 0,1 2
#   bash scripts/run_market_osnet_fdmf_stripe_mamba.sh summary ./logs/Market/my_output
set -euo pipefail
...
CONFIG="${CONFIG:-configs/Market/mambavision_tiny_osnet_fdmf_msef_stage_fcu_b64k4.yml}"
OUTPUT_BASE="${OUTPUT_BASE:-${OUTPUT_BASE_ARG:-./logs/Market/tiny/osnet_fdmf_stripe_mamba_no_osbbm}}"
...
declare -a EXPERIMENTS=(
  "fdmf_global_d1|0|4|True|1"
  "fdmf_global_d2|0|4|True|2"
  "fdmf_stripe_s1_g0_shared|1|4|True|0"
  "fdmf_stripe_s1_g1_shared_k2|1|2|True|1"
  "fdmf_stripe_s1_g1_shared|1|4|True|1"
  "fdmf_stripe_s1_g1_shared_k8|1|8|True|1"
...
run_experiment() {
  ...
  if [ "${SKIP_COMPLETED}" = "1" ] && [ -f "${train_log}" ] && grep -q "Validation (Regular) Results - Epoch:[[:space:]]*${COMPLETE_EPOCH}" "${train_log}"; then
    echo "[MarketStripeMamba] SKIP completed EXP=${name} output=${output_dir} epoch=${COMPLETE_EPOCH}"
    return 0
  fi
  ...
  echo "[MarketStripeMamba] GPU=${gpu} EXP=${name} output=${output_dir} stripe_depth=${stripe_depth} stripe_num=${stripe_num} stripe_share=${stripe_share} global_depth=${global_depth} osbbm=False"
...
summarize_results() {
  local specs
  specs="$(printf "%s\n" "${EXPERIMENTS[@]}")"
  echo "[MarketStripeMamba] Summarizing results -> ${SUMMARY_TSV} and ${SUMMARY_CSV}"

  MARKET_STRIPE_MAMBA_SPECS="${specs}" python - "${OUTPUT_BASE}" "${SUMMARY_TSV}" "${SUMMARY_CSV}" <<'PY'
import csv
import os
import re
import sys
...
specs = [line for line in os.environ.get("MARKET_STRIPE_MAMBA_SPECS", "").splitlines() if line.strip()]
...
if [ "${SUMMARY_ONLY}" -eq 1 ]; then
  echo "[MarketStripeMamba] OUTPUT_BASE=${OUTPUT_BASE}"
  summarize_results
  exit 0
fi

echo "[MarketStripeMamba] OUTPUT_BASE=${OUTPUT_BASE}"
echo "[MarketStripeMamba] Summary files: ${SUMMARY_TSV}, ${SUMMARY_CSV}"
echo "[MarketStripeMamba] SKIP_COMPLETED=${SKIP_COMPLETED} COMPLETE_EPOCH=${COMPLETE_EPOCH}"
...
summarize_results
echo "[MarketStripeMamba] All experiments finished."

if [ "${failures}" -ne 0 ]; then
  echo "[MarketStripeMamba] One or more experiments failed. Check the logs above and ${OUTPUT_BASE}."
  exit 1
fi
