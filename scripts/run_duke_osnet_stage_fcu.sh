...
run_experiment() {
  ...
  echo "[DukeOSNetStageFCU] GPU=${gpu} EXP=${name} output=${output_dir} stages=${stages} direction=${direction} init=${init_scale} osnet_w=${osnet_weight} fused_w=${fused_weight}"
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" python train.py --config_file "${CONFIG}" \
    ...
    MODEL.OSNET_FUSION.FCU_DIRECTION "'${direction}'" \
