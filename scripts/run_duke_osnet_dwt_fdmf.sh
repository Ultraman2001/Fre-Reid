#!/usr/bin/env bash
# Train DukeMTMC-reID MambaVision + OSNet orthogonal DWT frequency fusion.
# Usage:
#   bash scripts/run_duke_osnet_dwt_fdmf.sh 0,1 2
#   bash scripts/run_duke_osnet_dwt_fdmf.sh 0,1 2 ./logs/Duke/my_output
#   bash scripts/run_duke_osnet_dwt_fdmf.sh summary ./logs/Duke/my_output
set -euo pipefail
...
CONFIG="${CONFIG:-configs/DukeMTMC/mambavision_tiny_osnet_dwt_fdmf_b64k4.yml}"
OUTPUT_BASE="${OUTPUT_BASE:-${OUTPUT_BASE_ARG:-./logs/Duke/osnet_dwt_fdmf}}"
...
run_experiment() {
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" python train.py --config_file "${CONFIG}" \
    ...
    MODEL.OSNET_FUSION.FUSION_TYPE "'dwt_fdmf'" \
...
summarize_results() {
  ...
  specs="$(printf "%s\n" "${EXPERIMENTS[@]}")"
  echo "[DukeOSNetDWTFDMF] Summarizing results -> ${summary_file} and ${summary_csv}"

  DUKE_OSNET_DWT_FDMF_SPECS="${specs}" python - "${OUTPUT_BASE}" "${summary_file}" "${summary_csv}" <<'PY'
import csv
import os
import re
import sys
...
specs = [line for line in os.environ.get("DUKE_OSNET_DWT_FDMF_SPECS", "").splitlines() if line.strip()]
...
if [ "${SUMMARY_ONLY}" -eq 1 ]; then
  echo "[DukeOSNetDWTFDMF] OUTPUT_BASE=${OUTPUT_BASE}"
  summarize_results
  exit 0
fi

echo "[DukeOSNetDWTFDMF] OUTPUT_BASE=${OUTPUT_BASE}"
echo "[DukeOSNetDWTFDMF] Summary files: ${SUMMARY_TSV}, ${SUMMARY_CSV}"
...
summarize_results
echo "[DukeOSNetDWTFDMF] All experiments finished."

if [ "${failures}" -ne 0 ]; then
  echo "[DukeOSNetDWTFDMF] One or more experiments failed. Check the logs above and ${OUTPUT_BASE}."
  exit 1
fi
