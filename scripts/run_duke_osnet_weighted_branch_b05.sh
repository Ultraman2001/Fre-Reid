#!/usr/bin/env bash
# Train DukeMTMC-reID MambaVision + OSNet weighted branch-normalized fusion.
# Usage:
#   bash scripts/run_duke_osnet_weighted_branch_b05.sh 0,1 2
#   bash scripts/run_duke_osnet_weighted_branch_b05.sh 0,1 2 ./logs/Duke/my_output
#   bash scripts/run_duke_osnet_weighted_branch_b05.sh summary ./logs/Duke/my_output
...
CONFIG="${CONFIG:-configs/DukeMTMC/mambavision_tiny_osnet_weighted_branch_b05_b64k4.yml}"
OUTPUT_BASE="${OUTPUT_BASE:-${OUTPUT_BASE_ARG:-./logs/Duke/osnet_weighted_branch_ablation}}"
...
declare -a EXPERIMENTS=(
  "weighted_branch_b05_osw1_fuw1|weighted_branch|0.5|1.0|1.0"
)

run_experiment() {
  local idx="$1"
  local spec="$2"
  local name fusion_norm fusion_beta osnet_weight fused_weight

  IFS='|' read -r name fusion_norm fusion_beta osnet_weight fused_weight <<< "${spec}"
  ...
  echo "[DukeOSNetWeighted] GPU=${gpu} EXP=${name} output=${output_dir} norm=${fusion_norm} beta=${fusion_beta} osnet_w=${osnet_weight} fused_w=${fused_weight}"

  CUDA_VISIBLE_DEVICES="${gpu}" python train.py --config_file "${CONFIG}" \
    MODEL.DEVICE_ID "'${gpu}'" \
    MODEL.OSNET_FUSION.ENABLED True \
    MODEL.OSNET_FUSION.FUSION_NORM "'${fusion_norm}'" \
    MODEL.OSNET_FUSION.FUSION_BETA "${fusion_beta}" \
...
summarize_results() {
  ...
  specs="$(printf "%s\n" "${EXPERIMENTS[@]}")"
  echo "[DukeOSNetWeighted] Summarizing results -> ${summary_file} and ${summary_csv}"

  DUKE_OSNET_WEIGHTED_SPECS="${specs}" python - "${OUTPUT_BASE}" "${summary_file}" "${summary_csv}" <<'PY'
import csv
import os
import re
import sys
...
specs = [line for line in os.environ.get("DUKE_OSNET_WEIGHTED_SPECS", "").splitlines() if line.strip()]

fields = [
    "name", "status", "fusion_norm", "fusion_beta", "osnet_weight", "fused_weight",
...
rows = []
for spec in specs:
    name, fusion_norm, fusion_beta, osnet_weight, fused_weight = spec.split("|")
...
print()
print(
    f"{'name':<34} {'status':<12} {'norm':<16} {'beta':<6} {'osw':<5} {'fuw':<5} "
    f"{'last_ep':<8} {'last_mAP':<8} {'last_R1':<8} "
    f"{'best_ep':<8} {'best_mAP':<8} {'best_R1':<8}"
)
for row in rows:
    print(
        f"{row['name']:<34} {row['status']:<12} {row['fusion_norm']:<16} "
        f"{row['fusion_beta']:<6} {row['osnet_weight']:<5} {row['fused_weight']:<5} "
...
if [ "${SUMMARY_ONLY}" -eq 1 ]; then
  echo "[DukeOSNetWeighted] OUTPUT_BASE=${OUTPUT_BASE}"
  summarize_results
  exit 0
fi

echo "[DukeOSNetWeighted] OUTPUT_BASE=${OUTPUT_BASE}"
echo "[DukeOSNetWeighted] Summary files: ${SUMMARY_TSV}, ${SUMMARY_CSV}"
...
summarize_results
echo "[DukeOSNetWeighted] All experiments finished."

if [ "${failures}" -ne 0 ]; then
  echo "[DukeOSNetWeighted] One or more experiments failed. Check the logs above and ${OUTPUT_BASE}."
  exit 1
fi
