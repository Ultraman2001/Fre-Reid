#!/usr/bin/env bash
# Train DukeMTMC-reID MambaVision + OSNet FDMF/MSEF + directional Stage-FCU
# with the optional Mid-level Part-Rest Fusion (MPRF) auxiliary branch.
# Usage:
#   bash scripts/run_duke_osnet_fdmf_msef_mprf.sh 0,1 2
#   bash scripts/run_duke_osnet_fdmf_msef_mprf.sh 0,1 2 ./logs/Duke/my_output
#   bash scripts/run_duke_osnet_fdmf_msef_mprf.sh summary ./logs/Duke/my_output
set -euo pipefail
...
OUTPUT_BASE="${OUTPUT_BASE:-${OUTPUT_BASE_ARG:-./logs/Duke/osnet_fdmf_msef_mprf}}"
...
# name|num_parts|part_dim|loss_weight|infer_weight
declare -a EXPERIMENTS=(
  "mprf_k4_p128_l03_w05|4|128|0.3|0.5"
  "mprf_k4_p064_l03_w05|4|64|0.3|0.5"
  "mprf_k4_p128_l01_w05|4|128|0.1|0.5"
  "mprf_k4_p128_l05_w05|4|128|0.5|0.5"
  "mprf_k6_p064_l03_w05|6|64|0.3|0.5"
)

run_experiment() {
  local idx="$1"
  local spec="$2"
  local name num_parts part_dim loss_weight infer_weight

  IFS='|' read -r name num_parts part_dim loss_weight infer_weight <<< "${spec}"
  ...
  if [ "${SKIP_COMPLETED}" = "1" ] && [ -f "${train_log}" ] && grep -q "Validation (Regular) Results - Epoch:[[:space:]]*${COMPLETE_EPOCH}" "${train_log}"; then
    echo "[DukeMPRF] SKIP completed EXP=${name} output=${output_dir} epoch=${COMPLETE_EPOCH}"
    return 0
  fi
  ...
  echo "[DukeMPRF] GPU=${gpu} EXP=${name} output=${output_dir} parts=${num_parts} part_dim=${part_dim} loss_w=${loss_weight} infer_w=${infer_weight}"
  ...
  CUDA_VISIBLE_DEVICES="${gpu}" python train.py --config_file "${CONFIG}" \
    ...
    MODEL.OSNET_FUSION.MPRF_ENABLED True \
    MODEL.OSNET_FUSION.MPRF_NUM_PARTS "${num_parts}" \
    MODEL.OSNET_FUSION.MPRF_PART_DIM "${part_dim}" \
    MODEL.OSNET_FUSION.MPRF_REDUCTION 4 \
    MODEL.OSNET_FUSION.MPRF_INIT_SCALE 0.1 \
    MODEL.OSNET_FUSION.MPRF_LOSS_WEIGHT "${loss_weight}" \
    MODEL.OSNET_FUSION.MPRF_INFER_WEIGHT "${infer_weight}" \
    TEST.FEAT_MODE "'mamba_fdmf_osnet_mprf'" \
...
summarize_results() {
  local specs
  specs="$(printf "%s\n" "${EXPERIMENTS[@]}")"
  echo "[DukeMPRF] Summarizing results -> ${SUMMARY_TSV} and ${SUMMARY_CSV}"

  DUKE_MPRF_SPECS="${specs}" python - "${OUTPUT_BASE}" "${SUMMARY_TSV}" "${SUMMARY_CSV}" <<'PY'
import csv
import os
import re
import sys
...
specs = [line for line in os.environ.get("DUKE_MPRF_SPECS", "").splitlines() if line.strip()]

fields = [
    "name", "status", "num_parts", "part_dim", "loss_weight", "infer_weight",
...
rows = []
for spec in specs:
    name, num_parts, part_dim, loss_weight, infer_weight = spec.split("|")
...
print()
print(
    f"{'name':<24} {'status':<12} {'parts':<6} {'pdim':<6} {'loss':<6} {'infer':<6} "
    f"{'last_ep':<8} {'last_mAP':<8} {'last_R1':<8} "
    f"{'best_ep':<8} {'best_mAP':<8} {'best_R1':<8}"
)
for row in rows:
    print(
        f"{row['name']:<24} {row['status']:<12} {row['num_parts']:<6} "
        f"{row['part_dim']:<6} {row['loss_weight']:<6} {row['infer_weight']:<6} "
...
if [ "${SUMMARY_ONLY}" -eq 1 ]; then
  echo "[DukeMPRF] OUTPUT_BASE=${OUTPUT_BASE}"
  summarize_results
  exit 0
fi

echo "[DukeMPRF] OUTPUT_BASE=${OUTPUT_BASE}"
echo "[DukeMPRF] Summary files: ${SUMMARY_TSV}, ${SUMMARY_CSV}"
echo "[DukeMPRF] SKIP_COMPLETED=${SKIP_COMPLETED} COMPLETE_EPOCH=${COMPLETE_EPOCH}"
