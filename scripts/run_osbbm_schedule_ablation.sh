#!/usr/bin/env bash
# Run OSBBM scheduling ablations for PADE + OSBBM-m2.
set -euo pipefail
...
OUTPUT_BASE="${OUTPUT_BASE:-./logs/OCC-Duke/osbbm_schedule_ablation}"
...
declare -a EXPERIMENTS=(
  "osbbm_m2_always|0.25|8|2|0.50|base|always|1|0|20|10"
  "osbbm_m2_cycle_10on10off|0.25|8|2|0.50|base|cycle|1|0|20|10"
  "osbbm_m2_warm_cycle_21_160|0.25|8|2|0.50|base|cycle|21|160|20|10"
  "osbbm_m2_warm_cycle_21_120|0.25|8|2|0.50|base|cycle|21|120|20|10"
  "osbbm_m2_mid_41_120|0.25|8|2|0.50|base|range|41|120|20|10"
)
...
run_experiment() {
  ...
  echo "[OSBBM-Schedule] GPU=${gpu} EXP=${name} schedule=${schedule} start=${start_epoch} end=${end_epoch} period=${period_epochs} on=${on_epochs}"
...
summarize_results() {
  ...
  specs="$(printf "%s\n" "${EXPERIMENTS[@]}")"
  echo "[OSBBM-Schedule] Summarizing results -> ${summary_file} and ${summary_csv}"

  OSBBM_SCHEDULE_SPECS="${specs}" python - "${OUTPUT_BASE}" "${summary_file}" "${summary_csv}" <<'PY'
import csv
import os
import re
import sys
...
specs = [line for line in os.environ.get("OSBBM_SCHEDULE_SPECS", "").splitlines() if line.strip()]
...
print()
print(
    f"{'name':<30} {'status':<12} {'sched':<8} {'start':<6} {'end':<5} "
    f"{'last_ep':<8} {'last_mAP':<8} {'last_R1':<8} "
    f"{'best_ep':<8} {'best_mAP':<8} {'best_R1':<8}"
)
for row in rows:
    print(
        f"{row['name']:<30} {row['status']:<12} {row['schedule']:<8} "
...
summarize_results
echo "[OSBBM-Schedule] All experiments finished."

if [ "${failures}" -ne 0 ]; then
  echo "[OSBBM-Schedule] One or more experiments failed. Check the logs above and ${OUTPUT_BASE}."
  exit 1
fi
