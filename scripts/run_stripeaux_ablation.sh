#!/usr/bin/env bash
# Run the minimal StripeAux ablation set, two experiments at a time by default.
set -euo pipefail

GPU_IDS="${1:-${CUDA_VISIBLE_DEVICES:-0}}"
MAX_JOBS="${2:-2}"

CONFIG="${CONFIG:-configs/OCC_Duke/mambavision_tiny_transreid_pam_padeaug_stripeaux_b64k4.yml}"
OUTPUT_BASE="${OUTPUT_BASE:-./logs/OCC-Duke/stripeaux_ablation}"
...
declare -a EXPERIMENTS=(
  "base_padeaug|False|2|gem|256|0.00|False|global"
  "s2_gem_d256_w002|True|2|gem|256|0.02|False|global"
  "s2_gem_d256_w005|True|2|gem|256|0.05|False|global"
  "s2_gawp_d256_w005|True|2|gawp|256|0.05|False|global"
  "s2_avg_d256_w005|True|2|avg|256|0.05|False|global"
  "s4_gem_d256_w005|True|4|gem|256|0.05|False|global"
  "s4_gawp_d256_w005|True|4|gawp|256|0.05|False|global"
  "s2_gem_d128_w005|True|2|gem|128|0.05|False|global"
)

run_experiment() {
  local idx="$1"
  local spec="$2"
  local name enabled num_stripes pooling feat_dim loss_weight use_triplet inference

  IFS='|' read -r name enabled num_stripes pooling feat_dim loss_weight use_triplet inference <<< "${spec}"

  local gpu="${GPUS[$((idx % ${#GPUS[@]}))]}"
  local output_dir="${OUTPUT_BASE}/${name}"

  echo "[StripeAux] GPU=${gpu} EXP=${name} stripes=${num_stripes} pool=${pooling} dim=${feat_dim} weight=${loss_weight} infer=${inference}"

  CUDA_VISIBLE_DEVICES="${gpu}" python train.py --config_file "${CONFIG}" \
    MODEL.DEVICE_ID "'${gpu}'" \
    MODEL.STRIPE_AUX.ENABLED "${enabled}" \
...
summarize_results() {
  ...
  specs="$(printf "%s\n" "${EXPERIMENTS[@]}")"
  echo "[StripeAux] Summarizing results -> ${summary_file} and ${summary_csv}"

  STRIPEAUX_SPECS="${specs}" python - "${OUTPUT_BASE}" "${summary_file}" "${summary_csv}" <<'PY'
import csv
import os
import re
import sys
...
specs = [line for line in os.environ.get("STRIPEAUX_SPECS", "").splitlines() if line.strip()]

fields = [
    "name", "status", "enabled", "stripes", "pooling", "feat_dim",
    "loss_weight", "triplet", "inference",
...
fallback_block_re = re.compile(r"(Validation .*Results|=== .*Results ===)")
...
def parse_records(log_path):
    ...
    with open(log_path, "r", encoding="utf-8", errors="ignore") as handle:
        for line in handle:
            ...
            if current is None:
                if fallback_block_re.search(line):
                    current = {"epoch": "NA"}
                else:
                    continue
...
rows = []
for spec in specs:
    name, enabled, stripes, pooling, feat_dim, loss_weight, triplet, inference = spec.split("|")
...
print()
print(
    f"{'name':<24} {'status':<12} {'pool':<7} {'stripes':<7} "
    f"{'last_ep':<8} {'last_mAP':<8} {'last_R1':<8} "
    f"{'best_ep':<8} {'best_mAP':<8} {'best_R1':<8}"
)
for row in rows:
    print(
        f"{row['name']:<24} {row['status']:<12} {row['pooling']:<7} {row['stripes']:<7} "
...
summarize_results
echo "[StripeAux] All experiments finished."

if [ "${failures}" -ne 0 ]; then
  echo "[StripeAux] One or more experiments failed. Check the logs above and ${OUTPUT_BASE}."
  exit 1
fi
